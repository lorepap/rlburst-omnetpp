...
all_mean_fct = []
all_tail_fct = []
all_mean_background_fct = []
all_tail_background_fct = []
all_num_queries_completed_percentage = []
all_num_flows_completed_percentage = []
all_num_background_flows_completed_percentage = []
...
for category in CATEGORIES:
    ...
    mean_background_fct = []
    tail_background_fct = []
    percentage_num_queries_finished = []
    percentage_num_flows_finished = []  # Track flow completion per load
    percentage_background_num_finished = []  # Track background flow completion per load
    print("{}:\n".format(category))
    for ttl in TTLS:
                            ...
                            for incast_inter_arrival_mult in INCAST_INTER_ARRIVAL_MULT:
                                for incast_flow_size in INCAST_FLOW_SIZE:
                                    for incast_scale in NUM_REQUESTS_PER_BURST:
                                        ...
                                        if num_all_flows_requested == 0:
                                            percentage_num_requests_finished = 0
                                        else:
                                            percentage_num_requests_finished = num_all_flows_ended / num_all_flows_requested * 100
                                        ...
                                        mean_background_fct.append(np.mean(background_fct_cdf_info))
                                        tail_background_fct.append(np.percentile(background_fct_cdf_info, 99))
                                        # Add flow completion percentage for this load value
                                        percentage_num_flows_finished.append(percentage_num_requests_finished)
                                        # Add background flow completion percentage for this load value
                                        percentage_background_num_finished.append(percentage_background_num_requests_finished)
                                        
                                        print('Mean FCT, tail FCT\n{}, {}'
                                            .format(np.mean(fct_cdf_info),
                                        ...
                                        # Calculate percentage once per experiment (outside query_pack loop)
                                        if total_queries > 0:
                                            completion_percentage = (num_queries_completed / total_queries) * 100
                                        else:
                                            completion_percentage = 0
                                        percentage_num_queries_finished.append(completion_percentage)

                                        print('Mean QCT, tail QCT\n{}, {}'
                                                .format(np.mean(qct_info),
                                                        np.percentile(qct_info, 99)))
                                        print('Query completion percentage: {}%'.format(completion_percentage))
                                        print()
    ...
    all_mean_background_fct.append(mean_background_fct)
    all_tail_background_fct.append(tail_background_fct)
    all_num_flows_completed_percentage.append(percentage_num_flows_finished)
    all_num_queries_completed_percentage.append(percentage_num_queries_finished)
    all_num_background_flows_completed_percentage.append(percentage_background_num_finished)
    print('Percentage of background flows completed: {}'.format(percentage_background_num_finished))
    print('Total number of queries: {}'.format(total_queries))
    print('Percentage of queries completed: {}'.format(percentage_num_queries_finished))
    print()
...
plot_figs(x_values, all_num_flows_completed_percentage, CATEGORIES, markers, colors)
plt.xlabel('Load (%)')
plt.ylabel('Percentage of flows completed')
plt.savefig("figs/{}.png".format('simple_num_flows_completed_percentage'))
plt.close()

plot_figs(x_values, all_num_queries_completed_percentage, CATEGORIES, markers, colors)
plt.xlabel('Load (%)')
plt.ylabel('Percentage of queries completed')
plt.savefig("figs/{}.png".format('simple_num_queries_completed_percentage'))
plt.close()

# background flows completed
plot_figs(x_values, all_num_background_flows_completed_percentage, CATEGORIES, markers, colors)
plt.xlabel('Load (%)')
plt.ylabel('Percentage of background flows completed')
plt.savefig("figs/{}.png".format('simple_num_background_flows_completed_percentage'))
plt.close()

#background mean FCT
plot_figs(x_values, all_mean_background_fct, CATEGORIES, markers, colors)
plt.xlabel('Load (%)')
plt.ylabel('Mean background FCT (s)')
plt.savefig("figs/{}.png".format('simple_mean_background_fct'))
plt.close()
